...
def GaussianElimination(A):
    ...
    for i in range(0, n):
        ...
        maxEl = abs(A[rows[i],cols[i]])
        maxRow = i
        for k in range(i+1, n):
            if abs(A[rows[k],cols[i]]) > maxEl:
                maxEl = abs(A[rows[k],cols[i]])
                maxRow = k

        # Swap maximum row with current row (column by column)
        for k in range(i, n+1):
            tmp = A[rows[maxRow],cols[k]]
            A[rows[maxRow],cols[k]] = A[rows[i],cols[k]]
            A[rows[i],cols[k]] = tmp

        # Make all rows below this one 0 in current column
        for k in range(i+1, n):
            print("A[i][i]:",A[rows[i],cols[i]])
            if (A[rows[i],cols[i]] == 0):
                c = 0
            else:
                c = -A[rows[k],cols[i]]/A[rows[i],cols[i]]
            for j in range(i, n+1):
                if i == j:
                    A[rows[k],cols[j]] = 0
                else:
                    A[rows[k],cols[j]] += c * A[rows[i],cols[j]]
    ...
    for i in range(n-1, -1, -1):
        ...
        if (A[rows[i],cols[i]] == 0):
            ...
            x[i] = A[rows[i],cols[n-1]]/A[rows[i],cols[i]]
        for k in range(i-1, -1, -1):
            A[rows[k],cols[n-1]] -= A[rows[k],cols[i]] * x[i]
    return x
...
def rowdict2mat(rowdict):
    ...
    """
    col_labels = value(rowdict).D
    return Mat((list(keys(rowdict)), col_labels), {(r,c):rowdict[r][c] for r in keys(rowdict) for c in col_labels})
